...
func physicsUpdate(delta:float):
	...
	Player.transform.basis = Player.transform.basis.rotated(Vector3.UP, turn_input * ship_statemachine.ship_stats.drift_turn_speed * delta)
	

	# Roll the body based on the turn input
	ShipContainer.rotation.z = lerp(ShipContainer.rotation.z, turn_input*ship_statemachine.ship_stats.hovering_rollMultiplier*2, ship_statemachine.ship_stats.hovering_level_speed * delta)
	...
	if proxy.is_on_floor():
		...
		elif proxy.rotation.x > ship_statemachine.ship_stats.fallingPitchMax:
			var _remap_range = Helpers.Map(proxy.rotation.x, 0, ship_statemachine.ship_stats.fallingPitchMax, 0, 1)
