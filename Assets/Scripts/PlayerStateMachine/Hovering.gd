...
func physicsUpdate(delta:float):
	...
	Player.transform.basis = Player.transform.basis.rotated(Vector3.UP, turn_input * ship_resource.ship_stats.hovering_turn_speed * delta)
	...
	if Player.is_on_floor():
		...
		elif Player.rotation.x > ship_resource.ship_stats.fallingPitchMax:
			var _remap_range = Helpers.Map(Player.rotation.x, 0, ship_resource.ship_stats.fallingPitchMax, 0, 1)
