...
func _physics_process(delta: float) -> void:	
	...
	if not correctingRoll:
		...
		var targetRotation:Quaternion = Quaternion(targetBasis.orthonormalized()).normalized()
		var currentRotation:Quaternion = Quaternion(Player.basis.orthonormalized()).normalized()
