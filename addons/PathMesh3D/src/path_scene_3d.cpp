...
void PathScene3D::set_scale(const Vector3 &p_scale) { 
    if (scale != p_scale) {
        scale = p_scale;
        queue_rebuild();
    }
...
Vector3 PathScene3D::get_scale() const { 
    return scale; 
}
...
void PathScene3D::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_scale", "scale"), &PathScene3D::set_scale);
    ClassDB::bind_method(D_METHOD("get_scale"), &PathScene3D::get_scale);
    ADD_PROPERTY(PropertyInfo(Variant::VECTOR3, "scale", PROPERTY_HINT_RANGE, "0.0,100.0,0.01"), "set_scale", "get_scale");
